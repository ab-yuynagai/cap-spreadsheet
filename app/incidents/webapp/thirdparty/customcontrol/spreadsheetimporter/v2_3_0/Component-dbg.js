sap.ui.define(["sap/ui/core/UIComponent", "sap/ui/model/json/JSONModel", "sap/ui/Device", "./controller/SpreadsheetUpload", "sap/base/Log", "./controller/Logger", "sap/m/Button", "sap/ui/core/mvc/View", "./controller/Util", "./enums"], function (UIComponent, JSONModel, Device, __SpreadsheetUpload, Log, __Logger, Button, View, __Util, ___enums) {
  "use strict";
  ...
  const DefaultConfigs = ___enums["DefaultConfigs"];
  /**
   * @namespace cc.spreadsheetimporter.v2_3_0
   */
  const Component = UIComponent.extend("cc.spreadsheetimporter.v2_3_0.Component", {
    metadata: {
    ...
    createContent: function _createContent() {
      ...
      const buttonConfig = {
        text: buttonText,
        press: triggerDownload ? () => this.triggerDownloadSpreadsheet() : () => {
     ...
     * @param {ComponentData} [options] - Optional configuration overrides
     */
    openWizard: function _openWizard(options) {
      this._openImportDialog(true, options);
    },
    triggerDownloadSpreadsheet: async function _triggerDownloadSpreadsheet(deepDownloadConfig) {
      ...
      if (deepDownloadConfig) {
        this.setDeepDownloadConfig(Util.mergeDeepDownloadConfig(this.getDeepDownloadConfig(), deepDownloadConfig));
      }
     ...
     * @public
     */
    setArrayMessages: function _setArrayMessages(messagesArray) {
     ...
     */
    setReadSheetCoordinates: function _setReadSheetCoordinates(readSheetCoordinates) {
      if (typeof readSheetCoordinates === 'string') {
        ...
        try {
          ...
          // Fallback to default if conversion fails
          Log.error('Error converting coordinates to A1 notation', error, 'SpreadsheetUpload: Component');
          this.setProperty('readSheetCoordinates', 'A1');
        }
      } else {
        // Fallback to default
     ...
     * @private
     */
    _ensureContextAndEvents: function _ensureContextAndEvents() {
      if (!this.getContext()) {
     ...
     * @param {ComponentData} [options] - Optional configuration overrides
     */
    _openImportDialog: function _openImportDialog(useWizard, options) {
      // Ensure context and events are set up
      this._ensureContextAndEvents();
    ...
    getDirectUploadConfig: function _getDirectUploadConfig() {
      return this.getProperty('directUploadConfig');
    }
  });
  return Component;
});
