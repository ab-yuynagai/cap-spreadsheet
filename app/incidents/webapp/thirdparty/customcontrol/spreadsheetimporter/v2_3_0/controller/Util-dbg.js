sap.ui.define(["sap/ui/base/ManagedObject", "sap/base/Log", "sap/m/MessageBox", "sap/ui/base/ObjectPool", "sap/ui/base/Event", "cc/spreadsheetimporter/v2_3_0/thirdparty/xlsx"], function (ManagedObject, Log, MessageBox, ObjectPool, Event, XLSX) {
  "use strict";

  /**
   * @namespace cc.spreadsheetimporter.v2_3_0
   */
  const Util = ManagedObject.extend("cc.spreadsheetimporter.v2_3_0.Util", {
  ...
  Util.getLanguage = async function getLanguage() {
    try {
  ...
  Util.loadUI5RessourceAsync = async function loadUI5RessourceAsync(moduleName) {
  ...
  Util.mergeDeepDownloadConfig = function mergeDeepDownloadConfig(defaultConfig, providedConfig) {
    if (!providedConfig) return defaultConfig;

    // Deep merge for spreadsheetExportConfig
    const mergedDeepDownloadConfig = {
      ...defaultConfig,
      ...providedConfig
    };
    return mergedDeepDownloadConfig;
  };
  Util.mergeUpdateConfig = function mergeUpdateConfig(defaultConfig, providedConfig) {
    if (!providedConfig) return defaultConfig;
    const mergedUpdateConfig = {
      ...defaultConfig,
      ...providedConfig
    };
   ...
   */
  Util.convertCoordinatesToA1Notation = function convertCoordinatesToA1Notation(coordinates, includeEndCoordinate = false) {
    try {
      ...
      Log.error('Error converting coordinates to A1 notation', error, 'SpreadsheetUpload: Util');
      return null;
    }
  };
  /**
   ...
   */
  Util.convertA1NotationToCoordinates = function convertA1NotationToCoordinates(a1Notation) {
    try {
      ...
      Log.error('Error converting A1 notation to coordinates', error, 'SpreadsheetUpload: Util');
      return null;
    }
  };
  /**
   ...
   */
  Util.validateConfiguration = function validateConfiguration(componentData) {
    if (!componentData) {
    ...
    const knownProperties = ['spreadsheetFileName', 'action', 'context', 'columns', 'excludeColumns', 'tableId', 'odataType', 'mandatoryFields', 'fieldMatchType', 'activateDraft', 'batchSize', 'standalone', 'strict', 'decimalSeparator', 'hidePreview', 'previewColumns', 'skipMandatoryFieldCheck', 'skipColumnsCheck', 'skipMaxLengthCheck', 'showBackendErrorMessages', 'showOptions', 'availableOptions', 'hideSampleData', 'sampleData', 'spreadsheetTemplateFile', 'useTableSelector', 'readAllSheets', 'readSheet', 'spreadsheetRowPropertyName', 'continueOnError', 'createActiveEntity', 'i18nModel', 'debug', 'componentContainerData', 'bindingCustom', 'showDownloadButton', 'deepDownloadConfig', 'updateConfig'];
    ...
    if (componentData.columns && componentData.excludeColumns) {
      ...
      const excludedInColumns = componentData.excludeColumns.filter(col => columnsSet.has(col));
   ...
   */
  Util.deepCopyWorkbook = function deepCopyWorkbook(workbook) {
