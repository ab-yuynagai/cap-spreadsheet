sap.ui.define(["sap/base/Log", "./Util", "./Parser", "./services/FileService", "./services/DataExtractorService", "./services/ValidationService", "./services/UploadService"], function (Log, __Util, __Parser, __FileService, __DataExtractorService, __ValidationService, __UploadService) {
  "use strict";
  ...
  const DataExtractorService = _interopRequireDefault(__DataExtractorService);
  const ValidationService = _interopRequireDefault(__ValidationService);
  const UploadService = _interopRequireDefault(__UploadService);
  /**
  ...
  class ImportService {
    ...
    constructor(spreadsheetUploadController, component, i18nResourceBundle, messageHandler) {
     ...
     */
    async processFile(file, sheetOption, coordinates) {
      try {
        ...
        const extractResult = this.component.getStandalone() && this.component.getReadAllSheets() ? this.dataExtractorService.extractAllSheetsData(workbook) : this.dataExtractorService.extractSheetData(workbook, sheetName, coordinates || this.component.getReadSheetCoordinates());
        ...
        Log.error('Error processing file', error, 'ImportService');
        throw error;
      }
    }
     ...
     */
    async validateData(spreadsheetData, columnNames) {
     ...
     */
    async parseData(spreadsheetData) {
      ...
      const payloadArray = isStandalone ? spreadsheetData : Parser.parseSpreadsheetData(spreadsheetData, this.spreadsheetUploadController.typeLabelList, this.component, this.messageHandler, this.util, this.spreadsheetUploadController.isODataV4);
     ...
     */
    async processFileData(fileOrWorkbook, sheetOptionOrSheetName, coordinates) {
      try {
        let result;
        let file;
        ...
        Log.error('Error processing file data', error, 'ImportService');
        throw error;
      }
    }
     ...
     */
    async validateProcessedData(processedData, showMessages = false) {
      try {
        // Validate data
        ...
        Log.error('Error validating processed data', error, 'ImportService');
        throw error;
      }
    }
     ...
     */
    async executeUpload(payloadArray, file) {
     ...
     */
    async processAndValidate(fileOrWorkbook, sheetOptionOrSheetName, coordinates, options = {}) {
      const {
        resetMessages = true,
        validate = true,
        showMessages = false
      ...
      try {
        ...
        Log.error('Error in process and validate', error, 'ImportService');
        throw error;
      }
    }
     ...
     */
    async processValidateAndUpload(fileOrWorkbook, sheetOptionOrSheetName, coordinates, options = {}, callbacks = {}) {
      const {
        resetMessages = true,
        validate = true,
        showMessages = true,
      ...
      try {
        ...
        const result = await this.processAndValidate(fileOrWorkbook, sheetOptionOrSheetName, coordinates, {
          resetMessages,
          validate,
          showMessages: false
        } // Don't show messages in processAndValidate
        );
        ...
        Log.error('Error in process validate and upload', error, 'ImportService');
        throw error;
      }
    }
     ...
     */
    async runImportPipeline(fileOrWorkbook, sheetOptionOrSheetName, coordinates, pipelineOptions = {}, callbacks = {}) {
     ...
     * @returns Whether upload was successful
     */
    async determineAndExecuteUpload(payloadArray, file) {
     ...
     */
    async getWorkbookInfo(file, sheetOption) {
      const workbook = await this.fileService.readFile(file);
      const sheetName = await FileService.getSheetName(workbook, sheetOption || this.component.getReadSheet(), this.i18nResource);
      const rawSheetData = this.dataExtractorService.getRawSheetData(workbook, sheetName);
      return {
