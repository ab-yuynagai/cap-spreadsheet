sap.ui.define(["sap/ui/base/ManagedObject", "cc/spreadsheetimporter/v2_3_0/thirdparty/xlsx", "../Util", "sap/base/Log"], function (ManagedObject, XLSX, __Util, Log) {
  "use strict";
   ...
   */
  const SpreadsheetGenerator = ManagedObject.extend("cc.spreadsheetimporter.download.v2_3_0.SpreadsheetGenerator", {
    ...
    downloadSpreadsheet: async function _downloadSpreadsheet(entityDefinition, spreadsheetExportConfig) {
      ...
      try {
        const asyncEventBeforeDownloadFileExport = await Util.fireEventAsync('beforeDownloadFileExport', {
          workbook: wb,
          filename: filename
        }, this.component);
        isDefaultPrevented = asyncEventBeforeDownloadFileExport.bPreventDefault;
      } catch (error) {
        Log.error('Error while calling the beforeDownloadFileExport event', error, 'SpreadsheetGenerator.ts', 'downloadSpreadsheet');
      }
    ...
    _appendRootEntitySheet: async function _appendRootEntitySheet(wb, entityDefinition, spreadsheetExportConfig) {
    ...
    _appendSiblingsSheetsRecursively: async function _appendSiblingsSheetsRecursively(wb, entityDefinition, parentEntity, spreadsheetExportConfig) {
    ...
    _getSheet: function _getSheet(labelList, dataArray, columnsConfig) {
      ...
      worksheet['!ref'] = XLSX.utils.encode_range({
        ...
        e: {
          c: col - 1,
          r: dataArray.length + startRow
        }
      });
      worksheet['!cols'] = colWidths; // assign the column widths to the worksheet
      return worksheet;
    },
    _getCellForType: function _getCellForType(type, value) {
      switch (type) {
        case 'Edm.Boolean':
          return {
            v: value,
...
//# sourceMappingURL=SpreadsheetGenerator-dbg.js.map
