sap.ui.define(["sap/m/ColumnListItem", "sap/m/Table", "sap/m/Column", "sap/m/Text", "sap/ui/model/json/JSONModel", "sap/base/Log", "sap/m/MessageToast"], function (ColumnListItem, Table, Column, Text, JSONModel, Log, MessageToast) {
  "use strict";

  /**
   * HeaderSelectionStep – lets user pick the header row from the sheet preview.
   */
  class HeaderSelectionStep {
     ...
     */
    async handleHeaderSelection(rowIndex, coordinates) {
      try {
        ...
        this.wizard.processedData = await this.wizard.processFile(this.wizard.currentFile, this.wizard.getCurrentCoordinates(), true // Validate now that we have coordinates
        );
        ...
        const headerMessages = this.wizard.processedData.validationMessages.filter(message => message.type.title === 'EmptyHeaders');
        ...
        Log.error('Error handling header selection', error, 'HeaderSelectionStep');
        this.isValidated = false;
    ...
    validateStep(validated) {
      try {
        ...
        Log.error('Error validating step', error, 'HeaderSelectionStep');
      }
    }
     ...
     */
    createHeaderSelectionTable(rawSheetData, maxRows = 10) {
      try {
        ...
        // Add row number cell
        template.addCell(new Text({
          text: '{__metadata__/rowNumber}'
        }));
        ...
        Log.error('Error creating header selection table', error, 'HeaderSelectionStep');
        // Return an empty table as fallback
        return new Table();
      }
    }
  }
  return HeaderSelectionStep;
});
