sap.ui.define(["sap/ui/base/ManagedObject", "sap/base/Log", "./Util"], function (ManagedObject, Log, __Util) {
  "use strict";
   ...
   */
  const DirectUploader = ManagedObject.extend("cc.spreadsheetimporter.v2_3_0.DirectUploader", {
    /**
     ...
     */
    constructor: function _constructor(component, messageHandler, resourceBundle) {
     ...
     */
    uploadFile: async function _uploadFile(fileContent, fileName, entityNameBinding) {
      if (!this.config.enabled) {
        const error = new Error(this.util.geti18nText('spreadsheetimporter.directUploadNotEnabled'));
        Log.error('Direct upload not enabled', error, 'DirectUploader', () => this.component.logger.returnObject({
          config: this.config
        }));
        throw error;
      }
      ...
      if (this.config.uploadUrl) {
        ...
        if (!this.config.entityName && !entityNameBinding) {
          ...
          Log.error('No entity name provided for direct upload', error, 'DirectUploader', () => this.component.logger.returnObject({
            config: this.config
          }));
          throw error;
        }
        ...
        if (this.config.localhostSupport && window.location.hostname === 'localhost') {
          ...
          Log.info('Using localhost URL for direct upload', undefined, 'DirectUploader', () => this.component.logger.returnObject({
            baseUrl
          }));
        } else {
      ...
      try {
        ...
        Log.error('Error during direct upload', error, 'DirectUploader', () => this.component.logger.returnObject({
          error
        }));
     ...
     * @returns {Promise<any>} A promise that resolves with the server response.
     */
    uploadWithXHR: function _uploadWithXHR(url, fileContent, fileName) {
      return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            Log.info('Direct upload successful', undefined, 'DirectUploader', () => this.component.logger.returnObject({
              response
            }));
        ...
        try {
          ...
          Log.error('Error sending file', error, 'DirectUploader', () => ({
            fileSize: `${(fileContent.byteLength / (1024 * 1024)).toFixed(2)} MB`,
            fileName: fileName,
            contentType: this.config.contentType,
            error: error.message
